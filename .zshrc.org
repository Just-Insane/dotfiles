...
In VS Code terminals (and Copilot agent sub-shells), a minimal plugin set is used to avoid
network/daemon hangs: ~git-auto-fetch~ (SSH fetch on every prompt), ~github~ (API calls),
~rbw~ (Bitwarden daemon), ~dotenv~ (arbitrary ~.env~ sourcing), ~emacs~ (daemon socket),
and ~repo~ (network) are all excluded. ~pyenv~ is also excluded — pyenv init is handled
via the cached ~pyenv-init.zsh~ block, and the OMZ plugin triggers ~pyenv rehash~ which
acquires a file lock that can stall shell startup for 60 seconds.
...
Shell integration is sourced at the very end of this file (after P10K) so that its
~precmd~/~preexec~ hooks are appended after p10k's and not overwritten by them.
See [[*VS Code Shell Integration][VS Code Shell Integration]] below.
...
The agent launch and socket lookup are skipped in VS Code terminals — ~gpgconf~ can
hang for 60s if the agent is in a bad state. A static socket path is used instead.
...
The stale-lock cleanup removes ~.pyenv-shim~ on every shell start. This file is pyenv's
rehash mutex; if a ~pyenv rehash~ process is killed (e.g. by a timed-out terminal), the
lock is never released. Next startup, any new rehash waits 60 s before giving up — that
60 s wait is the visible hang. pyenv rehash completes in <5 s, so any surviving lock is
orthaned and safe to remove.
...
Auto-create the ~Default~ session with four general-purpose windows when a new terminal opens outside
of tmux. The ~has-session~ guard makes it a no-op if the session already exists. Skipped inside VS Code
terminals (which have their own integrated terminal experience).
...
Auto-create the ~claude~ session with four windows, each running ~claude --remote-control~ in its
project directory. This lets Dispatch route code tasks to named Claude Code sessions. Skipped if the
~claude~ CLI is not installed or if already inside tmux.
...
Must be sourced *after* P10K. P10K overwrites ~precmd~/~preexec~ hooks during its init; if shell
integration is sourced before P10K, its OSC 133 hooks are stripped and VS Code never receives the
"command finished" signal — every agent terminal command appears to hang.
...
** mssh — mosh auto-connect wrapper

Drop-in ~ssh~ replacement that transparently uses ~mosh~ for hosts listed in ~~/.mosh_hosts~. If the
host entry includes a tmux session name, it attaches (or creates) that session inside mosh.
Falls back to plain ~ssh~ for unknown hosts.
...
Adds the ~lms~ CLI (LM Studio's command-line tool) to PATH.
...
Adds ~tmuxinator~ gem bin to PATH and sources its zsh completion.
...
** Charm development signing identity

Keep local Charm builds signed with the stable self-signed identity so the Tauri app's Keychain ACL
survives rebuilds. Ad-hoc signing generates a new identity hash for each build.
...
Add Docker Desktop's completion directory to ~fpath~ and initialize completion support.
...
** Local Cargo test guard

Prevent routine local ~cargo test~ runs from regrowing the shared Cargo target directory. CI remains
the default test runner; set ~CARGO_TEST_LOCAL_OK=1~ for an intentional local run.
